...
LESS_TO_WIN_CATEGORIES = ['TO', 'GAA', 'GA', 'PF']
...
def get_comparison_stats(stats, categories, tiebreaker):
    comparison_stats = {}
    for team in stats:
        win_stat = Counter()
        for opp in stats:
            if opp != team:
                fake_result = get_pair_result(stats[team], stats[opp], categories, tiebreaker)
                win_stat[fake_result] += 1
        comparison_stats[team] = [win_stat['W'], win_stat['L'], win_stat['D']]
    return comparison_stats


def get_expected_score_and_result(stats, opponents_dict, categories, tiebreaker):
...
def get_pair_result(team_stat, opp_stat, categories, tiebreaker):
    win_count = 0
    lose_count = 0
...
def get_places_data(stats, categories):
...
def get_places_sum(matchup_pairs, categories):
    stats = get_stats(matchup_pairs)
    places_data = get_places_data(stats, categories)
